...
class TfIdfModel:
    def __init__(self):
        ...
        self.articles["raw_text"] = self.articles["raw_text"].map(
    ...
    def extract_unique_documents(dataset):
        ...
        combined_texts = current_texts.append(
            previous_texts, sort=False, ignore_index=True
        )
    ...
    def find_similar(self, index, top_n=5):
        cosine_similarities = linear_kernel(
            self.tf_idf_matrix[index : index + 1], self.tf_idf_matrix
        ).flatten()
    ...
    def calculate_cosine_pair_documents(
        self, target_article_title: str, source_article_title: str
    ):
        target_article_index = self.articles.loc[
            self.articles["article"] == target_article_title
        ].index[0]
        source_article_index = self.articles.loc[
            self.articles["article"] == source_article_title
        ].index[0]

        target_article_cosine_similarities = linear_kernel(
            self.tf_idf_matrix[target_article_index : target_article_index + 1],
            self.tf_idf_matrix,
        ).flatten()
