...
class NegativeSamplerProcessor:
    def __init__(self):
        ...
        self.dataset = ClickStreamPreProcessed().dataset

    def _generate_dataset(self):
        click_stream = self.click_stream_pre_processed.dataset
        click_stream = self.filter_click_stream_data(click_stream, self.get_available_titles_in_wiki_articles())
    ...
    def get_click_stream_dataset_with_negative_sampling(self):
        ...
        visited_articles = click_stream_pre_processed_dataset[["source_article", "target_article"]]
        ...
        combined_dataset = click_stream_pre_processed_dataset.append(negative_sampling, ignore_index=True)
    ...
    def run(
        ...
        train_articles = np.unique(
            train_dataset.dataset[["source_article", "target_article"]].iloc[train_dataset.indices].astype(str)
        )
        train_wiki = self.wiki_articles_pre_processed[self.wiki_articles_pre_processed["article"].isin(train_articles)]
        ...
        test_articles = np.unique(
            validation_dataset.dataset[["source_article", "target_article"]].iloc[test_dataset.indices].astype(str)
        )
        test_wiki = self.wiki_articles_pre_processed[self.wiki_articles_pre_processed["article"].isin(test_articles)]
        ...
        wiki_dataset.to_csv(
            os.path.join(save_folder, "dataset", "wiki_articles.csv"), index=False,
        )
    ...
    def generate_dataset_sample(
        ...
        unique_source_articles = self.dataset["source_article"].drop_duplicates().reset_index(drop=True)
        ...
        dataset_sample = self.dataset[self.dataset["source_article"].isin(selected_articles)].reset_index(drop=True)
        dataset_sample.to_csv(destination_path, index=False)

        self.generate_selected_articles(dataset_sample)

        return dataset_sample

    @staticmethod
    def generate_selected_articles(dataset):
        selected_articles = pd.Series.append(dataset["source_article"], dataset["target_article"])
