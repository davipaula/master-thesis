...
class ParagraphAttNet(nn.Module):
    def __init__(self, paragraph_hidden_size=50, sent_hidden_size=50, word_hidden_size=50, num_classes=14):
        super(ParagraphAttNet, self).__init__()

        self.paragraph_weight = nn.Parameter(torch.Tensor(2 * sent_hidden_size, 2 * sent_hidden_size))
        self.paragraph_bias = nn.Parameter(torch.Tensor(1, 2 * sent_hidden_size))
