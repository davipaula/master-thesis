...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        document_encode2 = [sentences[:self.max_length_word] for sentences in document_encode][
                          :self.max_length_sentences]

        document_encode3 = np.stack(arrays=document_encode2, axis=0)
        document_encode3 += 1

        return document_encode3.astype(np.int64), label
